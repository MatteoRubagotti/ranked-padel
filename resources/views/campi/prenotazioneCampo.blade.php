...
<div class="container first-component" id="div-prenotazione-campo">
  <div class="row pt-2 pe-lg-0 pt-lg-4 rounded-3 border shadow-lg">
    <div class="col-lg-4 col-sm-12 p-0 me-lg-3 me-5 ms-0 ms-lg-3 p-3">
      <div class="card card-cover h-100 overflow-hidden bg-light rounded-5"
        ...
        <div class="d-flex flex-column h-100 pt-2 pe-2 pb-3 ps-3 pb-0 text-shadow-1">
          <h4 class="mb-3 display-7 lh-1 fw-bold text-uppercase d-flex justify-content-end">{{$field->name}}</h4>
          ...
          <ul class="d-flex list-unstyled mt-auto mb-0">
            ...
            <li class="d-flex align-items-center me-3 text-white bg-info rounded-4 p-1 pe-2 ps-2">
              <i class="bi bi-umbrella me-2"></i></i><span class="text-uppercase"> Coperto </span>
              @else
            <li class="d-flex align-items-center me-3 text-white bg-warning rounded-4 p-1 pe-2 ps-2">
              <i class="bi bi-brightness-high me-2"></i></i><span class="text-uppercase"> Scoperto </span>
              @endif
            </li>
          </ul>
        </div>
    ...
    <div class="col-lg-7 ms-4 mt-3 text-center">
      <form name="prenota" method="post" action="{{ route('campi.prenota', ['idField' => $field->id]) }}" id="prenota-form">
        @csrf
        <h5><b class="text-uppercase"> Data: </b></h5>
        <label>
          <p class="mt-2"><input type="date" name="date" aria-required="Prova" id="date-picker" required>
            <span class="validity"></span></p>
        </label>

        <b><div id="date-invalid" class="text-danger"></div></b>

        <hr class="divider">

        <h5><b class="text-uppercase"> Orario: </b></h5>
        <span><button type="button" class="btn btn-secondary rounded-5 text-uppercase mb-3" onclick="checkDate({{ $field->id }})"><i class="bi bi-arrow-clockwise"></i> Aggiorna orari disponibili</button></span>
        <b><div id="date-wrong" class="text-danger"></div></b>

        <div class="btn-group p-2 w-75" role="group" aria-label="Orario mattina">
          <input type="radio" class="btn-check" name="time" value="08" id="btnradio08" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio08">08:00</label>

          <input type="radio" class="btn-check" name="time" value="09" id="btnradio09" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio09">09:00</label>

          <input type="radio" class="btn-check" name="time" value="10" id="btnradio10" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio10">10:00</label>

          <input type="radio" class="btn-check" name="time" value="11" id="btnradio11" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio11">11:00</label>

          <input type="radio" class="btn-check" name="time" value="12" id="btnradio12" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio12">12:00</label>
        </div>
        <div class="btn-group p-2 w-75" role="group" aria-label="Orario pomeriggio">
          <input type="radio" class="btn-check" name="time" value="14" id="btnradio14" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio14">14:00</label>

          <input type="radio" class="btn-check" name="time" value="15" id="btnradio15" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio15">15:00</label>

          <input type="radio" class="btn-check" name="time" value="16" id="btnradio16" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio16">16:00</label>

          <input type="radio" class="btn-check" name="time" value="17" id="btnradio17" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio17">17:00</label>

          <input type="radio" class="btn-check" name="time" value="18" id="btnradio18" autocomplete="off">
          <label class="btn btn-outline-success" for="btnradio18">18:00</label>
        </div>

        <b><div id="reservation-fail" class="text-danger font-weight-bolder"></div></b>

        <p class="mt-4 mb-4">
          <button type="submit" class="btn btn-primary text-uppercase me-3" id="btn-prenota" onclick="event.preventDefault(); checkHour({{ $field->id }})">Prenota</button>
