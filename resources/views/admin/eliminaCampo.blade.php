<form name="deleteField" method="post" action="{{ route('admin.eliminaCampo', ['idField' => $field->id]) }}">
    ...
    @else
    <button type="button" class="btn bg-danger text-white p-2 pt-0 pb-0 mt-2 ms-2" data-bs-toggle="modal"
        data-bs-target="#confirmDelete{{ $loop->index }}" style="" disabled>
        X
    </button>
    @endif
    <div class="modal fade" id="confirmDelete{{ $loop->index }}" tabindex="-1" aria-labelledby="Conferma eliminazione"
        aria-hidden="true">
