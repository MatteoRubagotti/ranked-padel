...
<div class="container first-component" id="div-prenotazione-campo">
  <div class="row pt-2 pe-lg-0 pt-lg-4 rounded-3 border shadow-lg">
    ...
    <div class="col-lg-7 ms-4 mt-3 text-center col-11">
      <form name="partecipa" method="post" action="{{ route('partite.partecipa', ['idGame' => $game->id]) }}">
        @csrf
        ...
        {{ $game->time }}
        <hr class="divider">
        <h5><b class="text-uppercase"> Proprietario: </b></h5>
        {{ $owner->username }}
        <hr class="divider">
        <h5><b class="text-uppercase"> Livello: </b></h5>
        <div class="ms-5 me-5">
          @if($game->level == 0)
          <h6
            class="mb-4 lh-1 fw-bold text-uppercase p-1 d-flex justify-content-center text-white text-center bg-info rounded">
            Principiante
            @elseif($game->level == 1)
            <h6
              class="mb-4 lh-1 fw-bold text-uppercase p-1 d-flex justify-content-center text-white text-center bg-primary rounded">
              Dilettante
              @elseif($game->level == 2)
              <h6
                class="mb-4 lh-1 fw-bold text-uppercase p-1 justify-content-center text-white bg-warning rounded">
                Campione
                @else
                <h6
                  class="mb-4 lh-1 fw-bold text-uppercase p-1 d-flex justify-content-center text-white text-center bg-danger rounded">
                  Leggenda
                  @endif
                </h6>
        </div>
        <p class="mt-4 mb-4">
          <button class="btn btn-primary text-uppercase me-3" id="btn-partecipa-partita">
            <input class="d-none" name="idUser" value="{{ Auth::id() }}" id="idUser"> <b>Partecipa</b> </button>
          <a href="{{ route('partite')}}" class="btn btn-danger text-uppercase text-white" id="btn-annulla">Annulla</a>
        </p>
